...
export default function MarkdownRenderer({ content, viewMode }: MarkdownRendererProps) {
  ...
  const bgColor = isIntuitive ? 'bg-white' : 'bg-[#1E293B]';
  const borderColor = isIntuitive ? 'border-gray-200' : 'border-[#334155]';
  const textColor = isIntuitive ? 'text-gray-700' : 'text-gray-300';
  const headingColor = isIntuitive ? 'text-gray-900' : 'text-gray-100';
  const subtleColor = isIntuitive ? 'text-gray-500' : 'text-gray-400';
